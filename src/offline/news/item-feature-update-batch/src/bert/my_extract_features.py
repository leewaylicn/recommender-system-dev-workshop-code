...
def get_recall_query_feat(cfg,vocab,recall_data,saved_model_dir):
    tf.reset_default_graph()
    # recall_data = load_recall(cfg.train_data_file)

    all_recall_feat = []
    recall_data_size = len(recall_data)
    recall_query = [item[1] for item in recall_data]
    logger.info('recall data size is: {}'.format(recall_data_size))

    recall_data_batch = data_batch_iter(recall_query, batch_size=cfg.batch_size, is_shuffle=False)

    with tf.Session(config=cfg.tf_config) as sess:
        ...
        for data in recall_data_batch:
            recall_quest_idx = encode_tokens2idx(vocab, data, cfg.num_unroll)
            recall_feat = sess.run('ori_q_feat:0',feed_dict={'ori_input:0':recall_quest_idx}).tolist()
            all_recall_feat.extend(recall_feat)

    all_recall_feat = np.array(all_recall_feat[:recall_data_size])

    # save to file, compressed
    logger.info('saving all recall feat to compressed npz file')
    np.savez_compressed(cfg.all_recall_feat_file,all_recall_feat=all_recall_feat)

    # load
    # np.load(cfg.all_recall_feat_file)['all_recall_feat']
    return all_recall_feat
